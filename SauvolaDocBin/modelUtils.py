...
def create_multiscale_sauvola(window_size_list=[7, 15, 23, 31, 39, 47, 55, 63],
                              train_k=True,
                              train_R=True,
                              train_alpha=True,
                              norm_type='inorm',
...
if __name__ == '__main__':
    model = create_multiscale_sauvola()

    data = np.ones([1, 256, 256, 1])

    tf.keras.utils.plot_model(model, show_shapes=True)
    predict = model.predict(data)
    print(predict[0].mean(), predict[1].mean(), predict[2].mean(), predict[3].mean())